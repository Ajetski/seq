export abstract class Sequenceable<T> {
	...
	concat(nextSequence: Sequenceable<T>): Concat<T> {
	...
	map<T2>(mappingFunc: (elem: T, idx: number) => T2): Map<T, T2> {
		return new Map(mappingFunc, this);
	}

	filter(filteringFunc: (elem: T, idx: number) => boolean): Filter<T> {
		return new Filter(filteringFunc, this);
	}

	fold<R>(init: R, operation: (acc: R, elem: T, idx: number) => R): R {
		let value = init;
...
export class Append<T> extends Sequenceable<T> {
	constructor(private baseSequence: Sequenceable<T>, private nextT: T) {
		super();
	}
...
export class Map<T, T2> extends Sequenceable<T2> {
	...
	forEach(cb: (elem: T2, idx: number) => unknown): void {
		let idx = 0;
		this.baseSequence.forEach((elem) => cb(this.mappingFunc(elem, idx), idx++));
	}
}
